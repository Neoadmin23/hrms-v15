...
from frappe.utils import getdate, logger, strip_html

logger.set_log_level("DEBUG")
logger = frappe.logger("hrmsv15", allow_site=True, file_count=1)
...
def get_address_data(address: str) -> dict:
    logger.info("get_address_data called")
...
@frappe.whitelist()
def office_address_list() -> list[dict]:
    logger.info("office_address_list called")
    addresses = frappe.get_all("Address", filters={"is_your_company_address": 1}, fields=["name", "address_line1", "address_title", "latitude", "longitude", "city", "state", "county"])
    logger.info(addresses)
    return addresses


@frappe.whitelist()
def get_geo_location_settings() -> dict:
    logger.info("get_geo_location_settings called")
    geo_loc_dict = frappe.get_single("Geo Location Settings").as_dict()
    logger.info(geo_loc_dict)
    return frappe.get_single("Geo Location Settings").as_dict()
